...
async fn load_test_concurrent_tasks() {
    ...
    println!("   Throughput: {:.2} tasks/sec", num_tasks as f64 / elapsed.as_secs_f64());
...
async fn load_test_node_capacity() {
    ...
    for i in 0..num_nodes {
        ...
        tasks.spawn(async move {
            let registration = NodeRegistration {
                ...
                node_type: if i % 4 == 0 { "compute" } else if i % 4 == 1 { "storage" } else if i % 4 == 2 { "gateway" } else { "validator" }.to_string(),
                capabilities: NodeCapabilities {
                    bandwidth_mbps: 1000.0,
                    cpu_cores: 8,
                    memory_gb: 16.0,
    ...
    println!("   Time elapsed: {:?}", elapsed);
    println!("   Throughput: {:.2} nodes/sec", num_nodes as f64 / elapsed.as_secs_f64());
...
async fn stress_test_mixed_operations() {
    ...
    println!("   Time elapsed: {:?}", elapsed);
    println!("   Operations/sec: {:.2}", (stats.total_nodes + task_count) as f64 / elapsed.as_secs_f64());
